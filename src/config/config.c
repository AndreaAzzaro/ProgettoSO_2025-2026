...
static ConfigurationKey resolve_configuration_key(const char *key_string) {
    ...
    while (configuration_mapping_table[i].string_key != NULL && found_key == KEY_UNKNOWN) {
        ...
        if (line_buffer[0] != '#' && line_buffer[0] != '\n' && line_buffer[0] != '\r') {
            ...
            if (equality_position != NULL) {
                *equality_position = '\0';
                ...
                /* BUG-24 FIX: Usa strtod() per preservare i decimali nei prezzi,
                 * poi converte a long per i campi interi */
                double double_value = strtod(value_part, NULL);
                long variable_value = (long)double_value;
                ...
                switch (identified_key) {
                    ...
                    /* BUG-24 FIX: Prezzi usano double_value per preservare la parte frazionaria */
                    case KEY_PRICE_PRIMI: configuration.prices.price_first_course = double_value; break;
                    case KEY_PRICE_SECONDI: configuration.prices.price_second_course = double_value; break;
                    case KEY_PRICE_COFFEE: configuration.prices.price_coffee_dessert = double_value; break;
